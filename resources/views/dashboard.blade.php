            ...
            @foreach(Auth::user()->staff->staffServices as $staffService)
                @foreach($staffService->service->consultancies->where('status','active') as $consultancy)
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-warning" data-toggle="modal" data-target="#send_{{$consultancy->id}}">Send Invesgation</button>
                            @include('patient.investigation')
                        </td>
                        <td>
                            <a href="{{route('patient.report',[$consultancy->patient->id])}}">
                            <button class ="btn btn-info">View Medical Hostory</button></a>
                        </td>
                    </tr>
                @endforeach
            @endforeach
        ...
        <tbody>
            @foreach(App\Models\Investigation::all() as $investigation)
                ...
                <tr>
                    ...
                    <td>{{$investigation->consultancy->patient->name}}</td>
                    <td>{{$investigation->consultancy->patient->hospital_no}}</a></td>
                    <td>{{$investigation->consultancy->patient->gender}}</td>
                    <td>{{$investigation->consultancy->patient->address}}</td>
                    <td>{{$investigation->consultancy->patient->contact}}</td>
                    <td>
